...
public class KindActionMappingEndpoint extends KAppNavEndpoint {
    private static final String className = KindActionMappingEndpoint.class.getName();
    ...
    @Path("/{resource-kind}")
    @Operation(
            summary = "Computes the config maps of a Kubernetes resource.",
            description = "Returns the computed configmap objects based on the Kubernetes resource information specified."
            )
    ...
    public Response getConfigmap( 
        ...
        try {
            ...
            if (Logger.isErrorEnabled()) {
                Logger.log(className, "getConfigMaps", Logger.LogType.ERROR, "Caught Exception returning status: " + getResponseCode(e) + " " + e.toString());
            }
    ...
    @Operation(
            ...
            description = "Returns the computed configmap objects for the given Kubernetes resource retrived from the cluster."
            )
    @APIResponses({@APIResponse(responseCode = "200", description = "OK"),
                   @APIResponse(responseCode = "207", description = "Multi-Status (Error from Kubernetes API)"),
                   @APIResponse(responseCode = "400", description = "Bad Request (Malformed input)"),
